...
- **Use as primary benchmark** — but confirm their evaluation protocol (train/test split vs. CV) before treating 88% as directly comparable to our cross-validated results.
...
- Broad survey of student performance prediction (SPP) methods — useful for framing the "related work" section and situating our model choices against the field's common approaches.
...
- Best-practices reference for academic-success prediction methodology — useful for justifying preprocessing/evaluation choices in the methods section.
...
## To search next (flagged, not yet retrieved)

- Papers specifically benchmarking ensemble methods (Random Forest, XGBoost, LightGBM) on *this exact* UCI dataset (id 856) — several exist on ResearchGate/IEEE per initial search; need full-text review before citing specific accuracy figures.
- Class-imbalance handling techniques appropriate for small-n, multi-class ordinal targets (e.g., SMOTE variants vs. cost-sensitive learning) — relevant given the Fail class has only 8/145 students.
- Whether GRADE should be modeled as nominal classification (current plan) or as ordinal regression (since 0–7 has a natural order) — worth a literature check before committing to a loss function family in Stage 3.

---
*This file is a living document — update citations with full bibliographic detail (DOI, page numbers) as each is properly read, not just discovered via search snippet.*
